<!--
  ...
  Effective version: 2026-09-24
  Source of Truth:   hakofront/src/pages/legal/archive/2026-09-24/
...
- **Desktop application update checks** — When automatic updates are enabled (the default; they can be turned off in the desktop settings), the desktop application connects to GitHub (`api.github.com` and its file download addresses) at startup and periodically while running, to look up and download new versions published by HakoSpace; the same applies when you check for updates manually. These connections carry no information about your account, your messages, or the servers you connect to; GitHub, under its own privacy statement, logs standard connection information such as your device's IP address. 
...
The HakoSpace desktop application (Electron) connects only to server addresses you configure and, when checking for and downloading software updates, to HakoSpace's software release source on GitHub (see "Other outbound connections" in §4).
 
 
- Authentication tokens are stored locally using platform-native encryption (`safeStorage` API) 
- No data is sent to HakoSpace servers (update checks and downloads only reach the public release source on GitHub, as described above) 
