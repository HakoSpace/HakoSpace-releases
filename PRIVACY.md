<!--
  ...
  Effective version: 2026-09-26
  Source of Truth:   hakofront/src/pages/legal/archive/2026-09-26/
...
**Network Logs**: While the HakoSpace software does not collect end-user IP addresses, when your server instance or desktop application communicates with our infrastructure (for opt-in telemetry, license verification, or checking for and downloading software updates), our standard web servers and CDN providers (e.g., Cloudflare) will temporarily log the public IP address of your server or device for security, DDoS protection, rate-limiting, and content-delivery purposes. We do not correlate these IP addresses with personal identity.
...
- **Desktop application update checks** — When automatic updates are enabled (the default; they can be turned off in the desktop settings), the desktop application connects to HakoSpace's software release source at startup and periodically while running, to look up and download new versions published by HakoSpace; the same applies when you check for updates manually. The release source is `dl.hakospace.com` (operated by HakoSpace using Cloudflare's storage and delivery services); when it is unavailable, the application connects to GitHub (`api.github.com` and its file download addresses) instead. These connections carry no information about your account, your messages, or the servers you connect to; standard connection information such as the IP address is handled as described under "Network Logs" above, and, for connections to GitHub, by GitHub under its own privacy statement. 
- **Server update checks** — Only when a server administrator manually checks for or downloads a software update in the server settings does the server connect to the same software release source (`dl.hakospace.com`, or GitHub when it is unavailable) to look up and download new versions published by HakoSpace. These requests carry only standard connection information and the server's software version, and no user data; the server's IP address is handled as described under "Network Logs" above, and, for connections to GitHub, by GitHub under its own privacy statement. Servers deployed from the official Docker image do not use this feature. 
...
The client, upon receiving this error, must display a **blocking modal** containing the full text of each document requiring re-acceptance. The modal has no dismissal path other than accepting or leaving the service. Each document requiring re-acceptance must be scrolled to the bottom in its full-text viewer before its checkbox can be checked; two independent checkboxes ("I have read and agree to the EULA" / "I have read and agree to the Privacy Policy") must be checked before the Accept button is enabled. Upon acceptance, the client POSTs the accepted version strings to the server, which updates the four account fields and grants access.
...
The HakoSpace desktop application (Electron) connects only to server addresses you configure and, when checking for and downloading software updates, to HakoSpace's software release source (`dl.hakospace.com`, or GitHub when it is unavailable; see "Other outbound connections" in §4).
 
 
- Authentication tokens are stored locally using platform-native encryption (`safeStorage` API) 
- No user data is sent to HakoSpace (update checks and downloads reach HakoSpace's software release source and carry only standard connection information, as described above) 
- The app itself has no built-in telemetry or crash reporting (for operating-system components, see below) 
- **Local cache**: To display content, the desktop application may cache received images, avatars, and stickers in the application data directory on your device. The game HUD feature uses a dedicated cache directory whose contents are deleted when the voice session ends and whenever the application starts or exits. Cached content is used only for local display and is not transmitted to any server. 
- **Game HUD (foreground window information)**: When the game HUD is enabled, the desktop application reads the title, process identifier, executable path, window identifier, on-screen position and size of the current foreground window, and the display it is on together with that display's scaling factor, solely to decide whether to show the HUD over that program and where to place it, to maintain the "disabled list" you configure (stored as executable paths in your local desktop settings until you remove them), and, when you choose "Share this game" from the HUD panel, to identify the window to be shared (the shared content itself is handled as described in Section 3.1 of this policy on screen sharing). This information is processed only on your device; it is not transmitted to HakoSpace or to the server you are connected to. 
- **Keyboard events**: To provide global shortcuts and push-to-talk, the desktop application receives keyboard events at the system level in order to match the key combinations you configure; this listening does not intercept or alter the keys that other applications receive. The game HUD itself does not listen to keyboard events at the system level; it only receives the shortcuts you configure for the HUD, through the operating system's hotkey registration (when you press such a key combination, it is received by the HUD). While the HUD panel is open, the panel becomes the foreground window and receives your keyboard input, just as any other window does when you switch to it; some programs may still detect keystrokes by other means, such as reading the keyboard state directly, which is beyond this software's control. The desktop application does not keep a record of your keystrokes and does not transmit keystroke data to any server; messages you deliberately send from the HUD input box are transmitted to the server you are connected to, the same as messages sent within the application. 
- **Operating-system components**: The game HUD is rendered by the Microsoft Edge WebView2 Runtime built into Windows. That component is part of the operating system; collection of its diagnostic data is governed by Microsoft's privacy statement and the Windows "Diagnostics & feedback" setting, and is not collected or transmitted by this software. 
