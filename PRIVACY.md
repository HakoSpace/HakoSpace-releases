<!--
  ...
  Effective version: 2026-09-18
  Source of Truth:   hakofront/src/pages/legal/archive/2026-09-18/
...
- **Game HUD (foreground window information)**: When the game HUD is enabled, the desktop application reads the title, process identifier, executable path, and on-screen position and size of the current foreground window, solely to decide whether to show the HUD over that program and where to place it, and to maintain the "disabled list" you configure (stored as executable paths in your local desktop settings until you remove them). This information is processed only on your device; it is not transmitted to HakoSpace or to the server you are connected to. 
